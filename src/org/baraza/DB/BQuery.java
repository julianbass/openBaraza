		...
		for(BElement el : view.getElements()) {
			...
			if(!colName.equals("")) {
				if(!fieldNames.contains(colName)) {
					...
					String userStrFnct = el.getAttribute("user_str_fnct");
					String titleFnct = el.getAttribute("title_fnct");
					String titleCol = el.getAttribute("title_col");

					if(el.getName().equals("ACTION")) function = null;
					if(!colNames.equals("")) colNames += ", ";
					if((titleFnct != null) && (titleCol != null)) colNames += "(" + titleFnct + ") as " + titleCol + ", ";
					if(function != null) colNames += "(" + function + ") as " + colName;
					else if(userFnct != null) colNames += userFnct + "," + user.getUserID() + ") as " + colName;
					else if(userStrFnct != null) colNames += userStrFnct + ", '" + user.getUserID() + "') as " + colName;
	...
	public void filterBuild(String wheresql, String orderby) {
		...
		if(view != null) {
			...
			if(view.getAttribute("group") != null) {
				String groupFilter = "(" + view.getAttribute("group") + " IN (" + user.getGroupIDs() + "))";
				if(wheresql == null) wheresql = "\nWHERE " + groupFilter;
			...
			if(view.getAttribute("link_filter") != null) {
				String groupFilter = "(" + view.getAttribute("link_filter") + " IN (SELECT link_to_id FROM entity_links WHERE entity_id = "
					+ user.getUserID() + "))";
				if(wheresql == null) wheresql = "\nWHERE " + groupFilter;
				else wheresql += " AND " + groupFilter;
			}
...
//System.out.println("BASE 1040 : " + el.getValue() + " : " + dataValue);
			...
			if(el.getAttribute("enabled","true").equals("false")) {
				...
				String defaultVal = db.getDefaultValue(el, user, linkData);
...
//System.out.println("BASE JSON : " + jsTb.toString());
    ...
    public String getCsvValueAt(int aRow, int aColumn) {
		...
		if(view != null) {
			...
			if(view.getAttribute("double.quote", "true").equals("false")) doubleQuote = false;
		}

		String mystr = "";
		if(myobj!=null) {
			if((myclass == String.class) && doubleQuote) {
			...
			} else if((myclass == String.class) && !doubleQuote) {
				if(hasQuote && myobj.toString().startsWith("0")) mystr = "'" + myobj.toString();
				else mystr = myobj.toString();

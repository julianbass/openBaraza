...
public class BDB {
	...
	public String getDefaultValue(BElement el, String linkData) {
		return getDefaultValue(el, user, linkData);
	}
	
	public String getDefaultValue(BElement el, BUser currUser, String linkData) {
		...
		String default_link_fnct = el.getAttribute("default_link_fnct");
		...
		} else if(default_link_fnct != null) {
			if(default_link_fnct.indexOf("(") > 1) {
				defaultValue = executeFunction("SELECT " + default_link_fnct + ", '" + linkData + "', '"
					+ currUser.getUserID() + "')");
			} else {
				defaultValue = executeFunction("SELECT " + default_link_fnct + "('" + linkData + "', '"
					+ currUser.getUserID() + "')");

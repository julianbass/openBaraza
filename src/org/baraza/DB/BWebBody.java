...
public class BWebBody extends BQuery {
	...
	public String getGrid(List<String> viewKeys, List<String> viewData, boolean addJSc, String viewKey, boolean sfield) {
		...
		if(view.getName().equals("APPROVALFORM")) hasSubs = false;

		if(view.getName().equals("FORMVIEW")  || view.getName().equals("APPROVALFORM")) {
		...
		try {
			...
			while (rs.next()) {
				...
				for(BElement el : view.getElements()) {
					String fieldTitle = el.getAttribute("title", "");
					String titleCol = el.getAttribute("title_col");
					if(el.getAttribute("lang") != null) {
						fieldTitle = translations.getTitle(orgId, languageId, el.getAttribute("lang"), fieldTitle);
					} else if(titleCol != null) {
						fieldTitle = rs.getString(titleCol);
					} else if(languageId > 0) { //if the language is not english
						fieldTitle = translations.getTitle(orgId, languageId, el.getValue(), fieldTitle);
					}
					...
					if(view.getName().equals("FORMVIEW") && !el.getValue().equals("")) {
						myhtml.append("\n<tr><td>" + fieldTitle + "</td>");
					} else if(view.getName().equals("APPROVALFORM") && !el.getValue().equals("")) {
						myhtml.append("\n<tr><td>" + fieldTitle + "</td>");
					}
					...
					if(!el.getValue().equals(""))  {
						...
						if (el.getName().equals("COLFIELD")) {
							...
							String mydv = "\n<td><a href='#";
							if(el.getAttribute("weblink") != null) {
								String weblink = rs.getString(el.getAttribute("weblink"));
    ...
    public String getForm(boolean isNew, String formLinkData, HttpServletRequest request) {
		...
		tab = "";
     	for(BElement el : view.getElements()) {
			...
			if(elName.equals("USERFIELD") || elName.equals("USERNAME") || elName.equals("REMOTEIP")) forForm = false;
			else if(elName.equals("DEFAULT") || elName.equals("FORMVIEW")) forForm = false;


			if(el.getAttribute("access") != null) {
				forForm = user.checkAccess(null, el.getAttribute("access"));
			}
	...
	public String getField(BElement el, String formLinkData, boolean eof, Integer formCols) {
		...
		String defaultValue = db.getDefaultValue(el, formLinkData);
		...
		if(el.getName().equals("HTML")) {
			...
			if(el.getAttribute("readonly", "false").equals("true")) response.append(" readonly ");
			...
			response.append("/>\n");
				...
				if(el.getAttribute("placeholder") != null) response.append("<label for='" + el.getAttribute("id") + "'>" + el.getAttribute("placeholder") + "</label>");
				response.append("<input class='form-control' type='file' name='" + el.getValue() + "' ");
				if(el.getAttribute("id") != null) response.append(" id='" + el.getAttribute("id") + "' ");
			...
			if(eof) response.append("<label>" + formatData(el) + "</label>\n");
		} else if(el.getName().equals("USERFIELD") || el.getName().equals("DEFAULT")) {
		}
	...
	public String getComboBox(BElement el, String formLinkData, boolean eof, String fieldValue, String defaultValue, String jsFunction) {
		...
		String noList = el.getAttribute("nolist");
		String noDefault = el.getAttribute("nodefault");
		...
		if(el.getAttribute("user") != null) {
			...
			String userFilter = "(" + el.getAttribute("nouser") + " != '" + user.getUserID() + "')";
			if(cmbWhereSql == null) cmbWhereSql = userFilter;
			else cmbWhereSql += " AND " + userFilter;
		}

		if(el.getAttribute("group") != null) {
			String groupFilter = "(" + el.getAttribute("group") + " IN (" + user.getGroupIDs() + "))";
			if(cmbWhereSql == null) cmbWhereSql = groupFilter;
			else cmbWhereSql += " AND " + groupFilter;
		}
...
//System.out.println("BASE COMBO : " + mysql);
		...
		if(noList == null) {		// Have an option not to display a list if its loaded by ajax
			BQuery cmbrs = new BQuery(db, mysql);
			while (cmbrs.moveNext()) {

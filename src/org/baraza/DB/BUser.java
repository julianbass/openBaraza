...
public class BUser {
	...
	public BUser(BDB db, String userIP, String userName, String userId) {
		...
		mData.put("login_ip", userIP);
		String keyFieldId = db.saveRec(insSql, mData);
	}
	...
	public BUser(BDB db, String userIP, String userName, String deviceId, String deviceSettings) {
		makeUser(db, userIP, userName);

		String insSql = "INSERT INTO sys_logins (entity_id, login_ip, device_serial_number, device_settings) VALUES (?, ?, ?, ?)";
		Map<String, String> mData = new LinkedHashMap<String, String>();
		mData.put("entity_id", userID);
		mData.put("login_ip", userIP);
		mData.put("device_serial_number", deviceId);
		mData.put("device_settings", deviceSettings);
		String keyFieldId = db.saveRec(insSql, mData);

		String updSql = "UPDATE entitys SET last_login = current_timestamp, locked_phone_id = '" + deviceId + "' WHERE entity_id = " + userID;
	...
	public void makeUser(BDB db, String userIP, String userName) {
		...
		if(orgID != null) {
			String wlSql = "SELECT org_name, department_filter, logo_path, lock_phone_login, "
			...
			if(rsOrg.moveNext()) {
				...
				lockPhoneLogin = rsOrg.getInt("lock_phone_login");
			}
			rsOrg.close();
	...
	public int getLockPhoneLogin() { return lockPhoneLogin; }

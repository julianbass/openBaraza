...
public class BSMS {
	...
	public BSMS(BDB db, BElement node, BLogHandle logHandle) {
		...
		String orgSQL = "SELECT org_id, sp_id, service_id, sender_name, sms_rate, "
			+ "send_fon, sc_register, ait_username, ait_app_key, alphanumeric, air_touch_key, provider_id, default_country_id "
		...
		while(orgRS.moveNext()) {
			...
			String[] orgParams = new String[12];
			...
			if("SO".equals(orgRS.getString("default_country_id"))) orgParams[11] = "252";
	...
	public void sendMessage() {
		...
		while(rs.moveNext()) {
			...
			String suffix = smsOrgs.get(orgID)[11];


			if(number == null) number = "";
			if(rs.getString("address_group_id") ==  null) {
				number = number.replace("\n", ",").replace("\r", "").replace("\"", "").replace("'", "").replace("/", "").replace("+", "").trim();
				if(number.startsWith("0")) number = suffix + number.substring(1, number.length());
				if(number.length() == 9) number = suffix + number;
				...
				if((numbers != null) && (numbers.trim().length() > 7)) {
					...
					for(String num : nums) {
						...
						if((num != null) && (num.length() > 3)) {
							num = num.replace("\n", ",").replace("\r", "").replace("\"", "").replace("'", "").replace("/", "").replace("+", "").trim();
							if(num.length() == 9) num = suffix + num;
							else if(num.startsWith("0")) num = suffix + num.substring(1, num.length());
	...
	public boolean sendSMS(String number, String message, String smsID, String orgID, boolean isRetry) {
		...
		while(retry != 0) {
			...
			} else if(smsOrgs.get(orgID)[10].equals("5")) { 		// For sending Somlink SMS
				sendResults = somlinkSend(number, message, orgID);
			} else {
	...
	public String hormuudSend(String number, String message, String orgID) {
		...
		System.out.println("Hormuud SMS Auth " + orgID + " " + userName + " " + apiKey);
	...
	public String somlinkSend(String number, String message, String orgID) {
		String resp = "";
		...
		System.out.println("Sonlink SMS Auth " + orgID + " " + userName + " " + apiKey);

		BSomlinkSMS somlink = new BSomlinkSMS(apiKey, userName, senderName);
		resp = somlink.sendSMS(number, message);

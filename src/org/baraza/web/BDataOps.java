...
public class BDataOps {
	...
	public JSONObject authenticate(HttpServletRequest request)  {
		...
		String authDeviceID = request.getHeader("authDeviceId");
		if(authUser == null || authPass == null) {
			jResp.put("ResultCode", 1);
			jResp.put("ResultDesc", "Wrong username or password");
			return jResp;
		}
		if(authDeviceID == null) authDeviceID = "";

		authUser = new String(Base64.getDecoder().decode(authUser));
		authPass = new String(Base64.getDecoder().decode(authPass));
		authDeviceID = new String(Base64.getDecoder().decode(authDeviceID));
		String body = BWebUtils.requestBody(request);
		log.info("authDeviceID : " + authDeviceID);
		log.info("DeviceParams : " + body);

		String deviceSettings = null;
		if(body != null) {
			JSONObject jBody = new JSONObject(body);
			if(jBody.has("device_settings")) {
				JSONObject jDeviceSettings = jBody.getJSONObject("device_settings");
				deviceSettings = jDeviceSettings.toString();
			}
		}
		...
		if(authFunction == null) {
			try {
				...
				BUser user = new BUser(db, request.getRemoteAddr(), authUser, authDeviceID, deviceSettings);
		...
		//log.info("BASE 2010 : " + authUser + " : " + authPass + " : " + userId);
...
System.out.println("BASE 3010 : " + viewKey);
	...
	public JSONObject getMenu(HttpServletRequest request, String userId, BTranslations translations) {
		JSONObject jResp = new JSONObject();
		...
		jResp = getSubMenu(user, menuXml, 0, translations);
		
		return jResp;
	}
	
	public JSONObject getSubMenu(BUser user, BElement mel, int level, BTranslations translations) {
		JSONObject jResp = new JSONObject();
		...
		for(BElement el : mel.getElements()) {
			...
			if(hasAccess) {
				if(el.isLeaf()) {
					String fieldTitle = el.getAttribute("name", "");
					if(el.getAttribute("lang") != null) {
						...
						fieldTitle = translations.getTitle(orgId, languageId, langTag + "_menu_" + el.getValue(), fieldTitle);
					}

					Integer mKey = Integer.valueOf(el.getValue());
					JSONObject jField = new JSONObject();
					jField.put("key", mKey);
					jField.put("name", fieldTitle);

					if(el.getAttribute("dashboard") != null) jField.put("dashboard", el.getAttribute("dashboard"));
					if(el.getAttribute("icon") != null) jField.put("icon", el.getAttribute("icon"));
					if(el.getAttribute("path") != null) jField.put("path", el.getAttribute("path"));
					jTable.put(jField);
				} else {
					jTable.put(getSubMenu(user, el, level+1, translations));
				}
			}
		}
	...
	public JSONObject getDashboard(HttpServletRequest request, String userId) {
		...
		if(eLinks != null) {
			...
			for(BElement el : eLinks.getElements()) {
				...
				if(hasAccess) {
					...
					if(el.getAttribute("path") != null) jLink.put("path", el.getAttribute("path"));
		...
		if(eAttendance != null) {
			...
			if(hasAccess) {
				...
				BElement eGeoFence = eAttendance.getElementByName("GEOFENCE");
				if(eGeoFence != null) {
					BQuery gfRs = new BQuery(db, eGeoFence, null, null, user, false);
					if(gfRs.moveFirst()) jResp.put("geofence", gfRs.getJSON());
				}
			}
		}
	...
	public JSONObject getForm(HttpServletRequest request, String userId, BTranslations translations) {
		...
		for(BElement el : view.getElements()) {
			if(BWebUtils.canDisplayField(el.getName())) {
				...
				if(el.getAttribute("step") != null) jField.put("step", el.getAttribute("step"));
				if(el.getAttribute("defaultValue") != null) jField.put("defaultValue", el.getAttribute("defaultValue"));
				...
				if(user == null) {
					...
					String defaultValue = db.getDefaultValue(el, user, linkData);
...
System.out.println("BASE 3020 WHERE : " + whereSql);
	...
	public JSONObject readData(HttpServletRequest request, String userId) {
		...
		if((view.getName().equals("FORM")) && (keyData == null)) return jResp;
...
System.out.println("BASE 2020 : addAttendance : "  + body);
		if(body != null) {
			...
			String mySql = "SELECT add_access_logs(" + user.getUserID() + "," + jObj.getString("log_type")
				...
				+ "', '" + jObj.getBigDecimal("lat").toString() + "," + jObj.getBigDecimal("long").toString()
				+ "'::point, 2);";

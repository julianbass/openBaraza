...
<c:set var="mainPage" value="recruitment_detail.jsp" scope="page" />
...
<%
	...
	String curl = request.getRequestURL() + "?work_type=" + workType + "&link=" + link;
		
	BDB db = new BDB(dbConfig);
	String sql = "SELECT department_role_name FROM vw_intake WHERE intake_id = " + link;
	String wTitle = db.executeFunction(sql);
	db.close();
	if(wTitle == null) {
		wTitle = "";
	} else if(wTitle != null) {
		curl += "&title=" + wTitle;
		wTitle = URLEncoder.encode(wTitle, StandardCharsets.UTF_8.toString());
	}
	curl = URLEncoder.encode(curl, StandardCharsets.UTF_8.toString());
...
<head>
	...
	<meta content="<%= wTitle %>" name="description"/>
	...
	<link href="./assets/css/recruitment_detail.css?2020" rel="stylesheet" type="text/css"/>
						   ...
						   <!-- START OF SINGLE JOB  SECTION -->
							<section class="single_job">
								...
								<div class="row">
									<div class="small-12 large-6 columns">
										<div id="share">
											<a class="fb" href="https://facebook.com/sharer/sharer.php?url=<%=curl%>" target="_blank">f</a>
											<a class="tw" href="https://twitter.com/intent/tweet?url=<%=curl%>" target="_blank">t</a>
											<a class="fb" href="https://api.whatsapp.com/send?text=<%=wTitle%>%20<%=curl%>" target="_blank">wp</a>
											<a class="li" href="https://linkedin.com/shareArticle?url=<%=curl%>" target="_blank">in</a>
										</div>
									</div>
								</div>
								...
								<div class="row">
									<div class="small-12 large-6 columns">
										<label for="chk_concent">I consent to the terms below </label>
										<input type="checkbox" id="chk_concent" name="chk_concent" value="true">
									</div>
										
									<div class="small-12 large-6 columns">
<p></br><b>RECRUITMENT  CONSENT FORM</b></br></br>

<b>COLLECTION, USE AND DISCLOSURE OF PERSONAL DATA</b></br></br>

I hereby certify that the facts set forth in the completed employment application are true and complete to the best of my knowledge. I understand that if employed, falsified statements on this application may result in dismissal. I release and hold harmless any person, firm, or entity that discloses matters in accordance with this authorization, as well as from liability that might otherwise result from the request for use of and/or disclosure of any or all of the foregoing information. </br></br>

Your personal data will be collected and used by us for the following purposes and we may disclose your personal data to third parties where necessary for the following purposes:</br>

a) Assessing and evaluating your suitability for employment in any current or prospective position within the organization; and</br>

b) Verifying your identity and the accuracy of your personal details and other information provided.</br>
									</div>
								</div>
							</section>
						...
						<div class="portlet light bordered"  style="background-color: #f5f7f9;padding-top: 30px;">
							...
							<div class="portlet-body">
								<div class="scroller" style="height:460px" data-rail-visible="1" data-rail-color="yellow" data-handle-color="#a1b2bd">
									...
									<ol class="list">
										<li>Read and accept the concent details below</li>
										...
										<li>Ensure you supply your current and valid CV in word or pdf</li>
										<li>Click on Apply. This will generate your user credentials to the system on the next screen and also sends the login credentials to the email provided.</li>
										<li>Proceed to login in to the application portal and populate the required academic and employment history</li>
									</ol>
								</div>
							</div>
						</div>
...
<script type="text/javascript" src="./assets/js/recruitment_detail.js?1025"></script>
...
<script type="text/javascript">
	...
	function job_application() {
		...
		if(!is_concent.is(":checked")) {
			let concentError = "<div style='color:red'><i class='glyphicon glyphicon-remove'></i>For you to proceed read the RECRUITMENT CONSENT details and tick bellow to concent</div>";
			$('#job_apply_msg').html(concentError);	
		} else if(valid_email) {
			...
			$.ajax({
				...
				success: function (rData) {
					...
					if(rData.error == 0) {
						let job_id = urlParams.get('link');
						<c:if test="${param.work_type eq '1'}">
							let applyUrl = "./recruitment?tag=apply_job&email=" + rData.data.applicant_email
								+ "&entity_id=" + rData.data.keyfield + "&intake_id=" + job_id;
						</c:if>
						<c:if test="${param.work_type eq '2'}">
							let applyUrl = "./recruitment?tag=apply_internship&email=" + rData.data.applicant_email
								+ "&entity_id=" + rData.data.keyfield + "&internship_id=" + job_id;
						</c:if>
						<c:if test="${param.work_type eq '3'}">
							let applyUrl = "./recruitment?tag=apply_job&email=" + rData.data.applicant_email
								+ "&entity_id=" + rData.data.keyfield + "&intake_id=" + job_id;
						...
						let applyErrMsg = "<div style='color:red'><i class='glyphicon glyphicon-remove'></i>" + rData.msg + "</div>";
	...
	$("#btn_application_view").click(function(e) {
		let userName = $("#td_user_name").text();
		let password = $("#td_first_password").text();
		
		$.post("ajaxauth", { j_username: userName, j_password: password }).done(function( data ) {
			window.location.href = "index.jsp?view=10:0";
		});
